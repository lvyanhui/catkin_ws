...
Detector::Detector(const string& model_file,
  ...
  this->model_file = 9999;  
  std::cout << "model_file" <<model_file<< std::endl;
  std::cout << "model_file" <<weights_file<< std::endl;
}

void Detector::Detect(cv::Mat& img, std::vector<cv::Rect > &rects, std::vector<int > &class_id, std::vector<float > &confidences)
{
  std::cout << "img size: " << img.cols << "\n";
  int w = img.cols;
  int h = img.rows;
  Blob<Dtype>* input_layer = net_->input_blobs()[0];
